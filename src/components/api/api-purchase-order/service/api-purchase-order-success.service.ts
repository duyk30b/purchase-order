...
export class ApiPurchaseOrderSuccessService {
  ...
  async success(options: {
    ...
    rootList.forEach((i) => {
      if ([PurchaseOrderStatus.DELIVERING].includes(i.status)) {
    ...
    const poHistoryDtoList = rootList.map((po) => {
      const poHistoryDto: PurchaseOrderHistoryInsertType = {
        ...
        status: {
          before: po.status,
          after: PurchaseOrderStatus.SUCCESS,
        },
        content: PurchaseOrderHistoryContent.SUCCESS,
        time: new Date(),
      }
      return poHistoryDto
    })
    ...
    return { data: { ids }, message: 'msg.MSG_066' }
  }
}
